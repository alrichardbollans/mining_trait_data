...
queryGBIF <- function(species_name, user=NULL, email=NULL, pwd=NULL, gbif_download_dir=NULL, rank='species', kingdom='Plantae', status_ping=3, time_out=300, index=NULL, check_output="", verbose=TRUE){
  ...
  if(missing(species_name)){stop("Please provide a valid species name")}
  if (any(!is.character(species_name))){stop("Please provide a valid species name (none character)")}
  if (any(species_name=="")){
    stop("Please provide a valid species name (empty name)")
  }
