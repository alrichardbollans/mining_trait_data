...
npass_natural_products_input = pd.read_csv(os.path.join(_inputs_path, 'NPASSv2.0_download_naturalProducts_generalInfo.txt'),
                                           sep='\t')[['np_id', 'pref_name']]
...
def get_npass_metabolites():
    ...
    species_with_np_info = pd.merge(np_sp_pairs, npass_natural_products_input, on='np_id', how='left')
