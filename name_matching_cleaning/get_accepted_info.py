...
from name_matching_cleaning import get_wcvp_info_for_names_in_column, \
    get_knms_name_matches, id_lookup_wcvp, resolve_unmatched, clean_urn_ids
...
def _get_knms_matches_and_accepted_info_from_names_in_column(df: pd.DataFrame, name_col: str) -> pd.DataFrame:
    ...
    best_matches = find_best_matches_from_multiples(multiple_matches)
    ...
    unmatched_resolutions = resolve_unmatched(unmatched, 'submitted')
...
def find_best_matches_from_multiples(multiple_match_records: pd.DataFrame, families_of_interest=None) -> pd.DataFrame:
    """
